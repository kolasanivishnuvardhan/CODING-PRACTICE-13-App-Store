...
class AppStore extends Component {
  ...
  getFilteredAppsList = ()=>{
    ...
    const filteredAppsList = appsList.filter((eachApp)=>eachApp.category===activeTabId)
  ...
  render() {
    ...
    const filteredSearchInputAppsList = filteredAppsList.filter((eachApp)=>eachApp.appName.toLocaleLowerCase().includes(searchInput.toLocaleLowerCase()))
    return (
      <div className="bg-container">
        ...
        <ul className="appsList-ul">
          {filteredSearchInputAppsList.map(eachMap => (
            <AppItem eachAppDetails={eachMap} key={eachMap.appId} />
          ))}
        </ul>
      </div>
    )
